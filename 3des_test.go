...
// 3DES加密
func ThreeDesEncrypt(text, key []byte) ([]byte, error) {
	// 创建密码（根据密码加密）
...
// 3DES解密
func ThreeDesDecrypt(text, key []byte) ([]byte, error) {
	// 创建密码（根据密码解密）
	block, err := des.NewTripleDESCipher(key)
	if err != nil {
		return nil, err
...
func TestThreeDes(t *testing.T) {
	...
	str := "hello world"
	// 定义密码（8字节）
	key := []byte("123456781234567812345678") // 中文占3字节，3*5+1 = 16
	fmt.Println("原文本：", str, "，密码：", string(key))
	// 加密
	encrypted, err := ThreeDesEncrypt([]byte(str), key)
	if err != nil {
		panic(err)
	}
	fmt.Println("加密结果：", base64.StdEncoding.EncodeToString(encrypted))
	// 解密
	decrypted, err := ThreeDesDecrypt(encrypted, key)
	if err != nil {
		panic(err)
	}
